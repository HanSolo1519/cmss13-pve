/datum/human_ai_equipment_preset/cia
	faction = FACTION_MARINE

/datum/human_ai_equipment_preset/cia/undercover
	name = "CIA AI (UPP Civilian Clothing)"
	desc = "Armed with a suppressed M1911, carries a simple IFAK with gauze, ointment and an injector."
	path = /datum/equipment_preset/cia/ai/upp_civilian

/datum/human_ai_equipment_preset/cia/undercover/smg
	name = "CIA AI (UPP Civilian Clothing, SMG)"
	desc = "Armed with either a M39B/2, PPSH, PPS43, or Bizon and a suppresed M1911, carries a simple IFAK with gauze, ointment and an injector."
	path = /datum/equipment_preset/cia/ai/upp_civilian/smg

/datum/human_ai_equipment_preset/cia/operative
	name = "CIA AI (Tactical Operative)"
	desc = "Armed with a suppressed M46C and M1911, carries a tactical medical pouch."
	path = /datum/equipment_preset/cia/ai/operative


//
